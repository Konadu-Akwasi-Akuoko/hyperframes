 ...
 * should carry on disk + in Cloud Storage. Shared by `src/handler.ts` (chunk +
 * assemble output paths) and `src/sdk/renderToCloudRun.ts` (final output key
 * construction) so the two sides agree on what an mp4 looks like vs a
 * png-sequence.
 */
